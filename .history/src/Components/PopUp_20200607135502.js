...
const PopUpPokemonDetail = props => {
    const { pokemon } = props;
    // const pokemonHeight = pokemon.height.toString().length = 1 ? '0,' + pokemon.height : pokemon.height.toString()[0] + ',' + pokemon.height.toString()[1]
    const pokemonHeight = pokemon.height.toString().length = 1 ? 'tak' : 'no'
